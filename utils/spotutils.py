...
def login_to_spotify(username):
    ...
    """
    # Make sure you source the Env vars
...
def get_playlist_data(spotipy_object, user_id, playlist_id):
    ...
    # print(f'user id {user_id} playlist id {playlist_id}')
...
def get_dataframe(spotipy_object, playlist_data, label):
    ...
    for i in tqdm_notebook(
        range(0, len(track_ids), 50), desc="Downloading audio features..."
    ):
    ...
    for i in tqdm_notebook(range(len(track_ids)), desc="Parsing track features..."):
    ...
    track_features_df = (
        pd.DataFrame.from_dict(track_features, orient="index")
        .reset_index()
        .rename(columns={"id": "track_id"})
        .drop(columns=["index"])[TRACK_FEATURES]
    )
    ...
    track_energy = (
        track_features_df[["track_id", "energy"]]
        .set_index("track_id")
        .to_dict()["energy"]
    )
    track_tempo = (
        track_features_df[["track_id", "tempo"]]
        .set_index("track_id")
        .to_dict()["tempo"]
    )
    track_time_signature = (
        track_features_df[["track_id", "time_signature"]]
        .set_index("track_id")
        .to_dict()["time_signature"]
    )
    track_valence = (
        track_features_df[["track_id", "valence"]]
        .set_index("track_id")
        .to_dict()["valence"]
    )
    track_duration_ms = (
        track_features_df[["track_id", "duration_ms"]]
        .set_index("track_id")
        .to_dict()["duration_ms"]
    )
    track_key = (
        track_features_df[["track_id", "key"]].set_index("track_id").to_dict()["key"]
    )
    track_liveness = (
        track_features_df[["track_id", "liveness"]]
        .set_index("track_id")
        .to_dict()["liveness"]
    )
    track_speechiness = (
        track_features_df[["track_id", "speechiness"]]
        .set_index("track_id")
        .to_dict()["speechiness"]
    )
    track_danceability = (
        track_features_df[["track_id", "danceability"]]
        .set_index("track_id")
        .to_dict()["danceability"]
    )
    track_loudness = (
        track_features_df[["track_id", "loudness"]]
        .set_index("track_id")
        .to_dict()["loudness"]
    )
    track_instrumentalness = (
        track_features_df[["track_id", "instrumentalness"]]
        .set_index("track_id")
        .to_dict()["instrumentalness"]
    )
    track_acousticness = (
        track_features_df[["track_id", "acousticness"]]
        .set_index("track_id")
        .to_dict()["acousticness"]
    )

    for i in tqdm_notebook(range(len(track_ids)), desc="Building final dataframe..."):
