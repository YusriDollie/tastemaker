...
from .data_processer import split_data, get_x
...
def get_train_clusters(clustered_train_data, all_clusters):
    ...
    """
    return {
        c: [_id for _id in all_clusters[c] if _id in clustered_train_data]
...
def run_active_suite(default_song_data, train_data, validation_data, algos, strats):
    """
    run active suite
    """
    results = {}
    ...
    for alg in algos:
        ...
        for strat in strats:
            ...
            for i in range(len((accs))):
                print(str((int(accs[i] * 10000) / 100)) + "\t")
            print()
...
def run_clusters_suite(default_song_data, train_data, validation_data, train_clusters):
    """
    run clusters suite
    """
...
def run_benchmark(
    alg, default_song_data, train_data, validation_data, discard=None
    ...
    clf = classifiers.Classifier(alg, train_data, default_vectorizer, False)
    clf.fit()
...
def get_highest_benchmark(  # pylint: disable=too-many-locals
    ...
    svc_bench_unclustered, svc_bench_unclustered_clf = run_benchmark(
        "svc", default_song_data, default_training_data, default_validation_data
    )
    ...
    svc_bench_clustered, svc_bench_clustered_clf = run_benchmark(
        "svc", default_song_data, clustered_training_data, clustered_validation_data
    )
    ...
    lsvc_bench_unclustered, lsvc_bench_unclustered_clf = run_benchmark(
        "lsvc", default_song_data, default_training_data, default_validation_data
    )
    ...
    lsvc_bench_clustered, lsvc_bench_clustered_clf = run_benchmark(
        "lsvc", default_song_data, clustered_training_data, clustered_validation_data
    )
    ...
    sgd_bench_unclustered, sgd_bench_unclustered_clf = run_benchmark(
        "sgd", default_song_data, default_training_data, default_validation_data
    )
    ...
    sgd_bench_clustered, sgd_bench_clustered_clf = run_benchmark(
        "sgd", default_song_data, clustered_training_data, clustered_validation_data
    )
