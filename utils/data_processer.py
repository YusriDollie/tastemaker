...
def filter_features(data, discard):
    ...
    for id_ in data:
        ...
        out[id_]["features"] = {
            k: v for k, v in out[id_]["features"].items() if k not in discard
        }
...
def test_cluster_size(data, max_cluster):
    ...
    x_train = numpy.array([list(x["features"].values()) for x in train_data.values()])

    for i in range(2, max_cluster + 1):
        km_classifier = KMeans(
            i, init="random", max_iter=300, random_state=0, n_init=30
        )
...
def get_kmeans_clusters(
    data, n_clusters, songs_by_cluster={}
    ...
    km_classifier = KMeans(
        n_clusters, init="random", max_iter=300, random_state=0, n_init=30
    )
    km_classifier.fit(x_train)

    cluster_map = pandas.DataFrame()

    cluster_map["data"], cluster_map["cluster"] = (
        x_train.tolist(),
...
def get_xy(song_data, ids=None):
    ...
    return [x["features"] for x in song_data.values()], [
        x["label"] for x in song_data.values()
    ]
