...
class Classifier:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        if algorithm not in constants.SUPPORTED_ALGS:
            raise ValueError("Unsupported algorithm: " + str(algorithm))
        if is_active:
            if al_strat not in constants.AL_STRATS:
        ...
        if clusters and algorithm not in constants.ONLINE_ALGS:
        ...
        self._is_online = algorithm in constants.ONLINE_ALGS
        ...
        if not self.is_active():
            self.x_train, self.y_train = data_processer.get_xy(self.train_data)
            ...
            if not self.uses_cluster_sampling():
                ...
                self.unseen_ids = {
                    i: list(self.clusters[i]) for i in range(self.num_clusters)
                }
    ...
    def active_learn(self, clusters=1, init=False):
        ...
        if not self.is_online():
            sample_x, sample_y = data_processer.get_xy(
                self.train_data, ids=self.train_ids
            )
            self.learner = get_default_learner(self.algorithm)
            self.learner.fit(self.transform(sample_x), sample_y)
        else:
            sample_x, sample_y = data_processer.get_xy(self.train_data, ids=sample_ids)
            if init:
                self.learner.partial_fit(
                    self.transform(sample_x), sample_y, constants.CLASSES
                )
            else:
    ...
    def al_sample(self, number, init):
        ...
        if strategy == "random":
            ...
            if init and not self.is_online():
                sampled_classes = set()
                while len(sampled_classes) != len(constants.CLASSES):
                    sampled_classes = set()
            ...
            unseen_x = data_processer.get_x(self.train_data, ids=self.unseen_ids)
            ...
            # self.unseen_ids
            sample_ids = [self.unseen_ids.pop(numpy.argmin(numpy.abs(unseen_x_scores)))]
    ...
    def al_sample_clusters(self, num_clusters, init):
        ...
        for i in range(self.num_clusters):
            ...
            if strategy == "random":
                ...
                unseen_x = data_processer.get_x(self.train_data, ids=self.unseen_ids[i])
                ...
                # self.unseen_ids
                sample_ids.append(
                    self.unseen_ids[i].pop(numpy.argmin(numpy.abs(unseen_x_scores)))
                )
    ...
    def validate(self, validation_data):
        ...
        """
        x_test, y_test = data_processer.get_xy(validation_data)
